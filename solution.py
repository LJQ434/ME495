...
class SOLUTION:
    ...
    def __init__(self,nextAvailableID):
        ...
        ###
        self.weightslayer1 = 2 * (numpy.random.rand(self.limbnumber, 2, self.hiddenneuron) -1)                                # weight of synapses (limb, 2 sensor, 4 hidden)
        self.weightslayer2 = 2 * (numpy.random.rand(self.limbnumber, self.hiddenneuron,  max(self.number_of_limbbody)) -1)     # weight of synapses (limb, 4 hidden, n motor)
    ...
    def Generate_Brain(self,ID):
        ...
        for limb in range (self.limbnumber):
            ...
            for currentRow in range(2): #name of sensor neurons
                for currentColumn in range(4*limb, 4*limb+2): #name of hidden neurons
                    pyrosim.Send_Synapse( sourceNeuronName = currentRow +2*limb,
                    targetNeuronName = currentColumn +  sensorneuron,
                    weight = self.weightslayer1[limb][currentRow][currentColumn] )
            #sym 
            for currentRow in range(2): #name of sensor neurons
                for currentColumn in range(4*limb+2, 4*limb+4): #name of hidden neurons
                    pyrosim.Send_Synapse( sourceNeuronName = currentRow +2*limb + 4,
                    targetNeuronName = currentColumn +  sensorneuron ,
                    weight = self.weightslayer1[limb][currentRow][currentColumn] )

            #layer2 (hidden to motor)
            for currentRow in range(4*limb , 4*limb+2): #name of hidden neurons
                for currentColumn in range(self.number_of_limbbody[limb]): #name of motor neurons
                    ...
                    targetNeuronName = currentColumn +  sensorneuron + self.hiddenneuron,
                    weight = self.weightslayer2[limb][currentRow][currentColumn] )
            #sym 
            for currentRow in range(4*limb+2 , 4*limb+4): #name of sensor neurons
                for currentColumn in range(self.number_of_limbbody[limb]): #name of motor neurons
                    pyrosim.Send_Synapse( sourceNeuronName = currentRow + sensorneuron,
                    targetNeuronName = currentColumn +  sensorneuron + self.hiddenneuron + self.number_of_limbbody[0]+self.number_of_limbbody[1],
                    weight = self.weightslayer2[limb][currentRow][currentColumn] )
    ...
    def Mutate(self,generation):
        ...
        self.weightslayer1[randlimb][randomRow][randomColumn] = 2 * numpy.random.random() - 1
        
        randlimb = numpy.random.randint(0,self.limbnumber)
        randomRow = numpy.random.randint(0,self.hiddenneuron)
        randomColumn = numpy.random.randint(0,self.number_of_limbbody[randlimb])
        self.weightslayer2[randlimb][randomRow][randomColumn] = 2 * numpy.random.random() - 1
